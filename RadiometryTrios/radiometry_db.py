...
from . import Radiometry, BaseRadiometry, RadiometryGroup
...
class RadiometryDB:
    ...
    def load_radiometry(self, idx=None, folder=None, area=None, station=None, measurement=None,
        ...
        """
        if idx is not None:
            path = self.path/self.data.loc[idx]['Relative_Path']
        ...
        return RadiometryGroup.open_folder(path,
    ...
    def summary(self, by='Area', plot=False, bars=None, total=True, **kwargs):
        ...
        summary = summary.groupby(by=by).count()[bars].rename(columns={'Relative_Path': 'Radiometry'})

        # as the name of the column has changed, correct it in the bars list
        bars.pop(0)
        bars = ['Radiometry'] + bars

        summary.sort_values(by=bars, ascending=False, inplace=True)

        # add the total record
        if total:
            summary = summary.append(summary.sum().rename('Total'))
    ...
    def reprocess_database(self, qry_str=None, prefixes=None, reflectance=False, mean_values=False, graphs=False):
        ...
        def reprocess_db_callback(row):
            ...
            rdmtry = RadiometryGroup.open_folder(out_path, prefixes=prefixes, load_interpolated=True)
            ...
            if mean_values:
                rdmtry.save_mean_radiometry(use_subset=False)

            if graphs:
                rdmtry.save_radiometries_graph(use_subset=False)

        self.loop_database(funcs=reprocess_db_callback, qry_str=qry_str)

    def update_metadata(self, attrs=None, qry_str=None, section='Metadata'):
        attrs = listify(attrs) if attrs is not None else self.default_attrs
    ...
    def check_structure(base_dir, prefixes, sep='_', base_name='spectrum_LO', dir_level=0):
        ...
        df['Group'] = None
        df['River'] = df['Dir'].map(lambda x: x.parts[-3 + dir_level])
    ...
    def append_new_data(self, from_folder, copy_all_files=False, reprocess=False, overwrite=False,
        ...
        df['NewDir'] = self.path.as_posix() + '/' + df['River'] + '/' + df['Station'] + '/' + df['Start_Date'].astype(
        ...
        if reprocess:
            for idx, row in df.iterrows():
                rdmtry = RadiometryGroup.open_folder(row['NewDir'], prefixes=['Ed', 'Ld', 'Lu'])
                rdmtry.create_reflectance(min_wl=320, max_wl=950)
                rdmtry.save_radiometries(save_backup=False, use_subset=False)
                rdmtry.save_radiometries_graph(use_subset=False)
        ...
        def map_get_attributes(folder):
            rdmtry2 = RadiometryGroup.open_folder(folder)

            return pd.Series([rdmtry2.metadata.get('Metadata', 'ed_device'),
                              rdmtry2.metadata.get('Metadata', 'ld_device'),
                              rdmtry2.metadata.get('Metadata', 'lu_device'),
                              int(rdmtry2.metadata.get('Metadata', 'rrs_measurements'))])
        ...
        df['Station_Name'] = df['River'] + '-' + df['Station']
        df['Measurement'] = df['NewDir'].map(lambda x: Path(x).relative_to(self.path).name)

        # step 5 - Update the stations workbook
        for river, station in df.groupby(by=['River', 'Station']).count().index:
            # self.create_station(river, station)
            self.insert_station(pd.Series({'River': river,
                                           'Station': station,
                                           'Station_Name': '-'.join([river, station])}),
                                overwrite=overwrite)
    ...
    # Saving Functions
    def commit_workbook(self, workbook='data'):
        if workbook == 'stations':
            df = self.stations
        elif workbook == 'summertime':
            df = self.summertime
        else:
        ...
        append_df_to_excel(self.control_file, df, sheet_name=self.workbooks[workbook]['name'], startrow=0,
                           truncate_sheet=True)
    ...
    def commit_all(self, workbooks=None):
        ...
        workbooks = ['data', 'stations'] if workbooks is None else listify(workbooks)
